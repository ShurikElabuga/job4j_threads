...
class CacheTest {
    ...
    @Test
    public void whenConsecutiveUpdatesThenVersionsIncrease() throws OptimisticException {
        var base = new Base(1, "Base", 1);
        var cache = new Cache();
        cache.add(base);
        cache.update(new Base(1, "Updated1", 1));
        cache.update(new Base(1, "Updated2", 2));
        var result = cache.findById(1).get();
        assertThat(result.version()).isEqualTo(3);
        assertThat(result.name()).isEqualTo("Updated2");
    }
}